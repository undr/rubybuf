...
describe Rubybuf::Message::Field::Float do
  before(:each) do
    @field = Rubybuf::Message::Field::Float.new(:required, :balance, 1, {})
  end
  ...
  context ".write_to" do
    it "writes value to writer" do
      ...
      writer.rewind
      writer.read.should == "\000\000@A"
      writer = StringIO.new
      @field.write_to(writer, -12)
      writer.rewind
      writer.read.should == "\000\000@\301"
      writer = StringIO.new
      @field.write_to(writer, 123456.123)
      writer.rewind
      writer.read.should == "\020 \361G"
      writer = StringIO.new
      @field.write_to(writer, -123456.123)
      writer.rewind
      writer.read.should == "\020 \361\307"
    end
  end
  context ".read_from" do
    it "reads value from reader" do
      reader = StringIO.new("\000\000@A")
      @field.read_from(reader).should == 12
      reader = StringIO.new("\000\000@\301")
      @field.read_from(reader).should == -12
      reader = StringIO.new("\020 \361G")
      @field.read_from(reader).should be_close(123456.123, 0.01)
      reader = StringIO.new("\020 \361\307")
      @field.read_from(reader).should be_close(-123456.123, 0.01)
    end
  end
end